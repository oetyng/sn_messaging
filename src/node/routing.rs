...
pub struct RoutingMessage(#[serde(with = "serde_bytes")] pub Vec<u8>);
...
impl RoutingMessage {
    ...
    /// Convinience function to deserialize a 'RoutingMessage' from bytes received over the wire.
    /// It returns an error if the bytes don't correspond to a node message.
    pub fn from(bytes: Bytes) -> Result<Self> {
        let deserialized = WireMsg::deserialize(bytes)?;
        if let MessageType::RoutingMessage(msg) = deserialized {
            Ok(msg)
        } else {
    ...
    pub fn serialize(&self) -> Result<Bytes> {
        WireMsg::serialize_routing_msg(self)
    }
}
